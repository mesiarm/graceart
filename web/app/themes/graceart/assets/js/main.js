(function ($) {
    ...
    $('.product-thumb-slider-vertical').slick({
        infinite: true,
        // Arrows are hidden, so show enough thumbs to reach them all.
        slidesToShow: 4,
        slidesToScroll: 1,
        vertical: true,
        focusOnSelect: true,
    ...
    -----------------------------------*/
    $(document).on('change', '.graceart-variation-radio', function () {
        var $radio = $(this),
            $form = $radio.closest('.variations_form'),
            attributes = $radio.data('attributes') || {},
            availability = $radio.data('availability'),
            priceHtml = $radio.data('price-html');
        ...
        if (availability) {
            $('#graceart-product-availability .graceart-availability-value').text(availability);
        }

        // Variable products show the price inside the variant boxes only, so this
        // is a no-op there; it still keeps a summary price in sync when one exists.
        if (priceHtml) {
            $form.closest('.product-summery').find('.product-price').html(priceHtml);
        }
    });

    $('.variations_form').each(function () {
        $(this).find('.graceart-variation-radio:checked').trigger('change');
    });

    /*--
        Shipping Info Tabs
    -----------------------------------*/
    $(document).on('click', '.product-shipping-info__tab', function () {
        var $tab = $(this),
            $wrap = $tab.closest('.product-shipping-info'),
            country = $tab.data('graceart-shipping-tab');

        $wrap.find('.product-shipping-info__tab').removeClass('active');
        $tab.addClass('active');

        $wrap.find('.product-shipping-info__list').removeClass('active');
        $wrap.find('.product-shipping-info__list[data-graceart-shipping-panel="' + country + '"]').addClass('active');
    });
